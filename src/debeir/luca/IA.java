...
public class IA extends Player {
	...
	public ArrayList<String> generateList(int i, Coordinate c) {
		...
		coord.setCoord(mot);
		if (coord.validCoordinate() == true && this.allCoordinatedShip().contains(coord.getCoord()) == false) {
	...
	public ArrayList<String> generateListAttackUn(Coordinate c) {
		...
		coord = new Coordinate(null,false);
		coord.setCoord(mot);
		if (coord.validCoordinate() == true && this.getListAttack2().contains(coord.getCoord()) == false) {
			listPossible.add(mot);
		}
		j = 0;
		while(1!=j) {
